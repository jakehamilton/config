...
in
{
  options.plusultra.services.websites.lasersandfeelings = {
    ...
    package = lib.mkOption {
      ...
      default =
        project.inputs.lasersandfeelings-website.result.packages.${pkgs.stdenv.hostPlatform.system}.default;
    };

    domain = lib.mkOption {
      description = "The domain to serve the website on.";
      type = lib.types.str;
      default = "lasersandfeelings.com";
    };
  ...
  config = lib.mkIf cfg.enable {
    users = {
      users = lib.optionalAttrs (cfg.user == "lasersandfeelings") {
        lasersandfeelings = {
          inherit (cfg) group;
          isSystemUser = true;
        };
      ...
      groups = lib.optionalAttrs (cfg.group == "lasersandfeelings") { lasersandfeelings = { }; };
    };
